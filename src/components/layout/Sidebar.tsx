...
import { photoEditorNavItems } from '@/config/photoEditorNav';
...
export const Sidebar = ({ collapsed = false }: SidebarProps) => {
  ...
  const { setCurrentServicePath, getCurrentService, isAccountantService, isContentService, isCharacterService, isSupplyManagerService, isProductFillerService, isPriceBuilderService, isBusinessAgentService, isPhotoEditorService } = useServiceStore();
  ...
  useEffect(() => {
    ...
    if (location.pathname === '/' || location.pathname.startsWith('/batteries') || 
      ...
      if (isProductFillerService()) {
        ...
        setOpenItem(match ? match.title : '');
      } else if (isPhotoEditorService()) {
        const match = photoEditorNavItems.find((group) =>
          (group.subItems || []).some((sub) => !!sub.href && location.pathname.startsWith(sub.href))
        );
        setOpenItem(match ? match.title : '');
      } else {
    ...
    <aside className={cn(
      ...
      <nav className={cn(collapsed ? 'mt-6' : 'mt-10', 'flex-1')}>
        <ul>
          ...
          {isPhotoEditorService() && photoEditorNavItems.map((item) => (
            <li key={item.title} className="mb-2">
              <Link
                to={(item.subItems && item.subItems.length > 0 ? '#' : item.basePath) || '#'}
                onClick={(e) => {
