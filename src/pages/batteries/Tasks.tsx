...
import { useGetBatteryTasks } from '@/hooks/useGetBatteryTasks';
...
import { useAddBatteryTask } from '@/hooks/useAddBatteryTask';
...
import { useUpdateBatteryTask } from '@/hooks/useUpdateBatteryTask';
import { useDeleteBatteryTask } from '@/hooks/useDeleteBatteryTask';
...
const BatteryTasksPage = () => {
  ...
  const { data: paginatedData, isLoading } = useGetBatteryTasks(currentPage, ITEMS_PER_PAGE);
  const addTask = useAddBatteryTask();
  const updateTask = useUpdateBatteryTask();
  const deleteTask = useDeleteBatteryTask();

  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [currentTask, setCurrentTask] = useState<Partial<UploadTask> | null>(null);

  const tasks: UploadTask[] = paginatedData?.results || [];
  const totalPages = Math.ceil((paginatedData?.count || 0) / ITEMS_PER_PAGE);

  const handleSave = () => {
    if (currentTask && !currentTask.task_type) {
      currentTask.task_type = TaskTypeEnum.COMPETITORS;
    }
    if (currentTask) {
      const taskToSave = { ...currentTask };
      if (taskToSave.id) {
        updateTask.mutate(taskToSave as Partial<UploadTask> & { id: number });
      } else {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-bold">Управління завданнями для акумуляторів</h1>
            ...
            <TableBody>
              {tasks.map((task) => (
                <TableRow key={task.id}>
                  ...
                  <TableCell>{new Date(task.start_time).toLocaleString('uk-UA', { year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit'})}</TableCell>
...
export default BatteryTasksPage;
