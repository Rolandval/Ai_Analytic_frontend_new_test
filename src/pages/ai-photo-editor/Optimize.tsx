...
import { Sliders, Download, Upload, X, ArrowLeftRight } from 'lucide-react';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/Card';
import { Button } from '@/components/ui/Button';
import { Progress } from '@/components/ui/Progress';
import { useQualityEnhancement } from '@/hooks/useQualityEnhancement';
...
export default function QualityEnhancement() {
  const [images, setImages] = useState<ProcessedImage[]>([]);
  const [dragActive, setDragActive] = useState(false);
  const qualityEnhancementMutation = useQualityEnhancement();
  ...
  const processImage = async (imageId: string, file: File) => {
    ...
    try {
      const result = await qualityEnhancementMutation.mutateAsync({
        image: file,
        data: {
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-gray-600 dark:text-gray-400 mt-1">
                AI покращення якості зображень з шумоподавленням
              </p>
            </div>
          </div>
        </div>
        ...
        <Card className="mt-6">
          ...
          <CardContent>
            <ol className="list-decimal list-inside space-y-2 text-sm text-gray-600 dark:text-gray-400">
              ...
              <li>AI покращить якість з шумоподавленням та підвищенням різкості</li>
              <li>Порівняйте результат з оригіналом</li>
              <li>Завантажте покращене зображення</li>
            </ol>
          </CardContent>
        </Card>
      </div>
