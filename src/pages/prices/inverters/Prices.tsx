...
export const InverterPrices = () => {
  ...
  useEffect(() => {
    console.log('🔵 [INVERTERS] localFullName changed:', localFullName);
    isTypingRef.current = true;
    const timer = setTimeout(() => {
      console.log('⏰ [INVERTERS] Debounce timer fired, applying filter:', localFullName);
      handleFilterChange('full_name', localFullName || undefined);
      isTypingRef.current = false;
    }, 300);
    return () => {
      console.log('🧹 [INVERTERS] Cleanup: clearing timer');
      clearTimeout(timer);
      isTypingRef.current = false;
    };
  ...
  useEffect(() => {
    console.log('🔄 [INVERTERS] filters.full_name changed:', filters.full_name, 'isTyping:', isTypingRef.current, 'localFullName:', localFullName);
    if (!isTypingRef.current && filters.full_name !== localFullName) {
      console.log('⚠️ [INVERTERS] RESETTING localFullName to:', filters.full_name || '');
    ...
    <div className="container mx-auto p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4 p-4 bg-slate-800/50 rounded-lg border border-slate-700">
         <input
            ...
            value={localFullName}
            onChange={(e) => {
              console.log('⌨️ [INVERTERS] Input onChange:', e.target.value);
