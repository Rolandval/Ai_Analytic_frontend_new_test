...
import AdminTariffManagement from '@/components/profile/AdminTariffManagement';
import { 
  User, 
  Check,
  ExternalLink,
...
export default function ProfileDashboard() {
  ...
  // Стан для відображення різних секцій
  const [activeSection, setActiveSection] = useState<'overview' | 'subscriptions' | 'admin'>('overview');
  
  // Мок-дані користувача (додаємо роль для адміністратора)
  const [userProfile] = useState<UserProfile & { role?: string }>({
    id: 'user-1',
    name: 'Олександр Петренко',
    email: 'alex.petrenko@example.com',
    plan: 'pro',
    joinDate: '2024-01-15',
    role: 'admin', // Додаємо роль адміністратора для демонстрації
    settings: {
      notifications: true,
      language: 'uk',
      theme: 'light'
  ...
  // Функція для рендерингу контенту залежно від активної секції
  const renderContent = () => {
    switch (activeSection) {
      case 'subscriptions':
        return <SubscriptionManagement />;
      case 'admin':
        return userProfile.role === 'admin' ? <AdminTariffManagement /> : null;
      default:
        return renderOverview();
    }
  };

  // Функція для рендерингу огляду профілю
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 p-6">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        {/* Макет з сайдбаром ліворуч */}
        <div className="grid grid-cols-1 lg:grid-cols-[260px,1fr] gap-6">
          <ProfileSidebar />
          <div className="space-y-8">
            {/* Динамічний контент залежно від активної секції */}
            {renderContent()}
          </div>
        </div>
      </div>
