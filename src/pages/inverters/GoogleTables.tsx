...
import { runInverterGoogleTableImport } from '@/services/inverterGoogleTables.api';
...
import {
  useGetInverterGoogleTables,
  useAddInverterGoogleTable,
  useUpdateInverterGoogleTable,
  useDeleteInverterGoogleTable,
} from '@/hooks/useInverterGoogleTables';
import { GoogleTable } from '@/types/googleTable';

const InverterGoogleTablesPage = () => {
  const { data = [], isLoading, refetch } = useGetInverterGoogleTables();
  const addMut = useAddInverterGoogleTable();
  const updMut = useUpdateInverterGoogleTable();
  const delMut = useDeleteInverterGoogleTable();
  ...
  const handleRunImport = async (id: number) => {
    try {
      ...
      const data = await runInverterGoogleTableImport(id);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-bold">Збережені таблиці (Інвертори)</h1>
...
export default InverterGoogleTablesPage;
