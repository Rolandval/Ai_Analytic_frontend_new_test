...
import { useGetSolarPanelSuppliers } from '@/hooks/useGetSolarPanelSuppliers';
import { useAddSolarPanelSupplier } from '@/hooks/useAddSolarPanelSupplier';
import { useUpdateSolarPanelSupplier } from '@/hooks/useUpdateSolarPanelSupplier';
import { useDeleteSolarPanelSupplier } from '@/hooks/useDeleteSolarPanelSupplier';
...
// Використовуємо стандартний Dialog для підтвердження видалення

const SolarPanelSuppliersPage = () => {
  const [currentPage, setCurrentPage] = useState(1);
  const [pageSize, setPageSize] = useState<number>(100);
  const [search, setSearch] = useState('');

  const { data: paginatedData, isLoading } = useGetSolarPanelSuppliers(currentPage, pageSize, search);
  const addSupplier = useAddSolarPanelSupplier();
  const updateSupplier = useUpdateSolarPanelSupplier();
  const deleteSupplier = useDeleteSolarPanelSupplier();
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <h1 className="text-3xl font-bold">Управління постачальниками сонячних панелей</h1>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            >
              {addSupplier.isPending || updateSupplier.isPending ? 'Збереження...' : 'Зберегти'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
...
export default SolarPanelSuppliersPage;
